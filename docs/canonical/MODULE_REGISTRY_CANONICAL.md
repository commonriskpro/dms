...
| `admin-core` | Implemented | Dealer-side role/permission admin flows, audits, file admin support, session switching support | `apps/dealer/modules/admin-core` | `/api/admin/*`, internal admin support | Canonical wrapper path. Legacy implementation alias remains `apps/dealer/modules/core-platform`. Dealer canonical permission catalog now lives in `apps/dealer/lib/constants/permissions.ts`. |
...
| `dashboard` | Implemented | Dashboard v3 data aggregation, layout persistence, metrics widgets | `apps/dealer/modules/dashboard` | `/api/dashboard/v3*`, `/api/dashboard/layout*` | Canonical surface is v3 (`getDashboardV3Data`, `/api/dashboard/v3`). Legacy v1 route removed in legacy migration sprint. Permission: `dashboard.read`; widget visibility depends on underlying domain permissions. |
| `dealer-application` | Implemented | Dealer application draft/submit flow, dealer-side activation linkage, and platform sync compatibility | `apps/dealer/modules/dealer-application` | `/api/apply/*`, `/api/internal/dealer-applications/[id]/platform-state` | Public intake still starts in dealer, but canonical review/onboarding state now lives in `apps/platform`. |
...
| `invite-bridge` | Implemented | Dealer invite lifecycle helpers used by the platform bridge | `apps/dealer/modules/invite-bridge` | dealer `/api/invite/*`, dealer `/api/internal/dealerships/*/invites*` | Canonical wrapper path. Legacy implementation alias remains `apps/dealer/modules/platform-admin`. It no longer owns dealer-hosted platform pages, public dealer `/api/platform/*` routes, or a dealer `PlatformAdmin` auth overlay. |
...
|---|---|---|---|---|
| `apps/websites` | Implemented | Public dealer website runtime; hostname-based tenant resolution; renders published releases only | `apps/websites/app/*` | Websites MVP complete. Spec: `apps/dealer/docs/WEBSITES_PUBLIC_RUNTIME_SPEC.md`. Scale-Up spec: `WEBSITES_SCALEUP_SPEC.md` (Step 1 complete). No dealer auth. Separate Vercel project. |
