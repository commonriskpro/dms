...
- **Front gross:**  
  - frontGrossCents = salePriceCents - purchasePriceCents - totalFeesCents - taxCents.  
  - Doc fee is part of totalFeesCents and thus reduces front gross. (If a business rule later excludes doc fee from reducing gross, use frontGrossCents = salePriceCents - purchasePriceCents - (totalFeesCents - docFeeCents) - taxCents; for this spec the standard formula applies.)
- **Trade:**  
  - NetTradeCents = allowanceCents - payoffCents (per DealTrade row).  
  - In this scope, trade does **not** reduce the taxable base: taxable base = salePriceCents + taxableFeesCents only. If jurisdiction requires trade allowance to reduce taxable sale price, extend formula to (salePriceCents - allowanceCents) + taxableFeesCents and document in business rules.
...
**Money in API:** All money fields in **request bodies** may be sent as numbers (dollars) from UI; server converts to cents before persist. **Responses** return money as **string** (e.g. `"12345"` for 123.45 dollars as 12345 cents). Document in API contract: response money fields are string (cents).
