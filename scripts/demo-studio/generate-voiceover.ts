/**
 * Cinematic Demo Studio — convert script to narration audio.
 * Uses ElevenLabs if ELEVENLABS_API_KEY exists; fallback to system TTS.
 * Output: /demo-output/voiceover.mp3. Fails gracefully if external API missing.
 */
...
function generateWithSystemTTS(text: string): void {
  ...
  try {
    ...
    const bin = ffmpeg && fs.existsSync(ffmpeg) ? ffmpeg : "ffmpeg";
    execSync(`${bin} -y -i "${aiffPath}" -acodec libmp3lame -q:a 2 "${VOICEOVER_PATH}"`, { stdio: "inherit" });
  } catch {
    if (fs.existsSync(aiffPath)) {
      fs.copyFileSync(aiffPath, VOICEOVER_PATH.replace(".mp3", ".aiff"));
    }
    return;
  }
  if (fs.existsSync(aiffPath)) fs.unlinkSync(aiffPath);
