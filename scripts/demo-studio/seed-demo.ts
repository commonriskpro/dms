/**
 * Cinematic Demo Studio — seed demo data (idempotent).
 * Populates: Vehicles (10), Leads (6), Deals (3).
 * Scenarios: aging vehicle, missing photos, lead pipeline stages, funding pending deal.
 * Does not modify application code. Requires DATABASE_URL and dealer Prisma schema.
 */
...
const schemaPath = path.join(dealerDir, "prisma/schema.prisma");
...
async function ensureDemoDealership(): Promise<{
...
async function ensurePipeline(dealershipId: string): Promise<{
  id: string;
...
async function clearDemoData(dealershipId: string): Promise<void> {
...
export async function runSeedDemo(): Promise<void> {
  ...
  // Leads (6) — spread across pipeline stages
...
async function main(): Promise<void> {
  try {
    await runSeedDemo();
    console.log("[seed-demo] Demo data seeded (10 vehicles, 6 leads, 3 deals).");
  } finally {
