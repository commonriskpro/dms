/**
 * AI demo pipeline — automated UI walkthrough.
 * Executes actions: open dashboard, inventory, add vehicle, CRM pipeline, move opportunity, deal desk, delivery queue.
...
/** Force dark theme for the demo (localStorage + document class). Call after first navigation, then reload. */
...
export async function runDemo(): Promise<void> {
  ...
  // Write scenes.json from registry
  fs.writeFileSync(
    path.join(OUT_DIR, "scenes.json"),
    JSON.stringify(SCENE_REGISTRY, null, 2),
    "utf-8"
  );
  console.log("[run-demo] Wrote demo-output/scenes.json");
  ...
  try {
    ...
    for (const scene of SCENE_REGISTRY) {
      console.log(`[run-demo] Scene: ${scene.id} (${scene.action}, ${scene.duration}s)`);
    ...
    if (video) {
      try {
        ...
        if (videoPath && fs.existsSync(videoPath)) {
          ...
          console.log("[run-demo] Video saved to", RAW_VIDEO);
        }
      } catch {
        // ignore
      }
