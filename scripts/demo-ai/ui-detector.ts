/**
 * AI demo pipeline — UI detection engine.
 * Uses Playwright DOM inspection to capture element position, width, height, bounding box.
 * Saves metadata to /demo-output/ui-map.json. Requires app to be running at DEMO_BASE_URL.
 */
...
export type UIElementMeta = {
  ...
  width: number;
  height: number;
  /** Bounding box: { x, y, width, height } (same as above, for compatibility). */
...
/** Resolve first matching selector (comma-separated selectors tried in order). */
...
export async function runUIDetector(): Promise<void> {
  ...
  try {
    ...
    for (const scene of SCENE_REGISTRY) {
      ...
      const box = await getBoundingBox(page, scene.selector);
      if (box) {
        elements.push({
          sceneId: scene.id,
