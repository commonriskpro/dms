/**
 * Demo pipeline — seed demo data (idempotent).
 * Populates: Vehicles (10), Leads/Opportunities (6), Deals (3), Alerts (2 via vehicle state).
 * Run from repo root with: dotenv -e .env.local -- npx tsx scripts/demo-pipeline/seed-demo.ts
 * Requires: DATABASE_URL and dealer Prisma client (npm run prisma:generate:dealer).
 */

import path from "path";
import { randomUUID } from "node:crypto";

// Use Prisma client for dealer schema (generated by prisma:generate:dealer; may be in root or apps/dealer)
...
const prismaClientPath =
  (() => {
    ...
    const dealerClient = path.join(dealerDir, "node_modules/@prisma/client");
    try {
      require.resolve(rootClient);
      return rootClient;
    } catch {
      try {
        require.resolve(dealerClient);
        return dealerClient;
      } catch {
        return rootClient;
      }
...
async function ensureDemoDealership() {
  ...
  if (!dealership) {
    ...
    console.log("  Created demo dealership.");
  }
...
async function ensurePipeline(dealershipId: string) {
  ...
  if (!pipeline) {
    ...
    console.log("  Created default pipeline with stages.");
...
/** Remove previous demo data so this run is idempotent. */
...
export async function runSeedDemo(): Promise<void> {
  console.log("[seed-demo] Ensuring demo dealership and location...");
  const { dealership, location } = await ensureDemoDealership();
  const dealershipId = dealership.id;
  const locationId = location.id;

  console.log("[seed-demo] Ensuring CRM pipeline and stages...");
  ...
  // Vehicle 1: ensure no photos (already created without photos)
  // Vehicle 2: aging (created with old createdAt)
  // Vehicle 3: in recon (VehicleRecon IN_PROGRESS)

  // --- Customers/Leads (6) and Opportunities ---
  console.log("[seed-demo] Creating 6 demo leads (customers + opportunities)...");
  ...
  // --- Deals (3): e.g. funding pending, delivered ---
  console.log("[seed-demo] Creating 3 demo deals...");
  ...
  console.log("[seed-demo] Done. Vehicles: 10, Leads: 6, Deals: 3, Alerts: 2 (missing photos + aging).");
}
