/**
 * Demo pipeline — orchestrate full demo generation.
 * Order: seed-demo → launch app → run-walkthrough → generate-script → generate-voiceover → render-video.
...
const DEMO_DIR = path.join(ROOT, "scripts", "demo-pipeline");
...
function waitForApp(timeoutMs: number): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tryLater = () => {
      if (Date.now() - start > timeoutMs) reject(new Error("App did not become ready in time."));
...
async function main() {
  ...
  console.log("Step 1/6: seed-demo");
  await runTsx("seed-demo.ts");
  console.log("");

  console.log("Step 2/6: launch app");
  ...
  console.log("Step 3/6: run-walkthrough");
  await runTsx("run-walkthrough.ts");
  appProcess.kill();
  console.log("");

  console.log("Step 4/6: generate-script");
  await runTsx("generate-script.ts");
  console.log("");

  console.log("Step 5/6: generate-voiceover");
  await runTsx("generate-voiceover.ts");
  console.log("");

  console.log("Step 6/6: render-video");
  await runTsx("render-video.ts");
  console.log("");

  console.log("Step 3/5: generate-script");
  await runTsx("generate-script.ts");
  console.log("");

  console.log("Step 4/5: generate-voiceover");
  await runTsx("generate-voiceover.ts");
  console.log("");

  console.log("Step 5/5: render-video");
  await runTsx("render-video.ts");
  console.log("");

  console.log("=== Done. Output: demo-output/product-demo.mp4 ===");
}
