...
describe("worker handlers", () => {
  ...
  it("bulk import worker posts to dealer internal bulk-import endpoint", async () => {
    ...
    expect(postDealerInternalJobMock).toHaveBeenCalledWith("/api/internal/jobs/bulk-import", {
      dealershipId: job.data.dealershipId,
      importId: job.data.importId,
      requestedByUserId: job.data.requestedByUserId,
      rowCount: job.data.rowCount,
      rows: job.data.rows,
    });
  ...
  it("analytics worker posts to dealer internal analytics endpoint", async () => {
    postDealerInternalJobMock.mockResolvedValue({
      dealershipId: "11111111-1111-1111-1111-111111111111",
      type: "sales_metrics",
      invalidatedPrefixes: [],
  ...
  it("alerts worker posts to dealer internal alerts endpoint", async () => {
    postDealerInternalJobMock.mockResolvedValue({
      dealershipId: "11111111-1111-1111-1111-111111111111",
      type: "alert_check",
      invalidatedPrefixes: [],
  ...
  it("vin decode worker posts to dealer internal vin-decode endpoint", async () => {
    postDealerInternalJobMock.mockResolvedValue({
      dealershipId: "11111111-1111-1111-1111-111111111111",
    ...
    const result = await processVinDecodeJob(job);
    ...
    expect(result.attachedDecode).toBe(true);
  });

  it("crm execution worker posts to dealer internal crm endpoint", async () => {
    postDealerInternalJobMock.mockResolvedValue({
      processed: 3,
      failed: 1,
      deadLetter: 0,
