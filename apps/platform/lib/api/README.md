...
- **Usage**: For `/api/platform/*` routes, wrap the handler so every response is logged and correlated:
  ```ts
  ...
  ```
- **Representative route**: `app/api/platform/bootstrap/route.ts` uses this pattern. Other platform API routes should use `withApiLogging` for consistent correlation IDs and structured logging.
