...
export async function POST(
  ...
  try {
    ...
    if (!checkPlatformRateLimit(clientId, "invite_owner")) {
    ...
    const result = await inviteOwnerForApplication(applicationId, user.userId);
    ...
    if (e instanceof DealerProvisionError) {
      return errorResponse(
        ...
        "Dealership could not be provisioned. Click \"Provision Dealership\" first, or check that the dealer app is reachable and DEALER_INTERNAL_API_URL / INTERNAL_API_JWT_SECRET are set correctly.",
        e.status >= 400 ? e.status : 502
      );
    }
    if (e instanceof DealerInviteError) {
      return errorResponse(
        "DEALER_INVITE_FAILED",
        e.message,
        e.status >= 400 ? e.status : 502
      );
    }
