...
export function toCustomerDetail(c: CustomerFromService): CustomerDetail {
  ...
  return {
    ...
    idIssuedDate:
      cAny.idIssuedDate instanceof Date ? cAny.idIssuedDate.toISOString().slice(0, 10) : (cAny.idIssuedDate ?? null),
    idExpirationDate:
      cAny.idExpirationDate instanceof Date
        ? cAny.idExpirationDate.toISOString().slice(0, 10)
        : (cAny.idExpirationDate ?? null),
...
function serializeCallback(row: CallbacksResult["data"][number]): CustomerCallbackItem {
  return {
    id: row.id,
