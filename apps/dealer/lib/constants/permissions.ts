...
/**
 * System roles created at dealership provision. Keys are role display names.
 * Permission keys must exist in DEALER_PERMISSION_CATALOG.
 */
export const DEFAULT_SYSTEM_ROLE_KEYS: Record<string, string[]> = {
  Owner: [...DEALER_PERMISSION_KEYS],
  Admin: DEALER_PERMISSION_CATALOG.filter((entry) => entry.key !== "admin.roles.write").map((entry) => entry.key),
  "General Manager": DEALER_PERMISSION_CATALOG.filter((entry) => entry.key !== "admin.roles.write").map((entry) => entry.key),
  "Sales Manager": [
    "dashboard.read",
    "notifications.read",
    "inventory.read",
