      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          {canAddVehicle ? (
            <Link
              href="/inventory/new"
              className="flex h-10 items-center justify-center rounded-[10px] bg-[var(--accent-inventory)] px-3 text-sm font-semibold text-white hover:opacity-90"
            >
              Add Vehicle
            </Link>
          ) : null}
          {canAddLead ? (
            <Link
              href="/customers/new"
              className="flex h-10 items-center justify-center rounded-[10px] bg-[var(--accent-leads)] px-3 text-sm font-semibold text-white hover:opacity-90"
            >
              Add Lead
            </Link>
          ) : null}
          {canStartDeal ? (
            <Link
              href="/deals/new"
              className="flex h-10 items-center justify-center rounded-[10px] bg-[var(--accent-deals)] px-3 text-sm font-semibold text-white hover:opacity-90"
            >
              Start Deal
            </Link>
          ) : null}
        ...
        <div className="overflow-hidden rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface-2)]">
          <TableToolbar
            search={
              <Input
                value={query}
            ...
            <Table>
              ...
              <TableBody>
                {filteredRows.map((row) => (
                  <TableRow key={row.id} className="hover:bg-[var(--surface)]/60">
                    ...
                    <TableCell className="tabular-nums">{toDaysInStock(row.createdAt)}</TableCell>
                    <TableCell>
