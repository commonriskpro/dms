...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = await analyticsRead.getAnalyticsSummary(ctx.dealershipId, from, to);
    return jsonResponse({ data: summary, from: from.toISOString(), to: to.toISOString() });
  } catch (e) {
