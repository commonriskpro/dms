...
import { dealerJobRunsDailyQuerySchema } from "@dms/contracts";
 ...
 * Service-to-service JWT required. Returns daily aggregate rows by dealership.
 */
export async function GET(request: NextRequest): Promise<Response> {
  ...
  const parsed = dealerJobRunsDailyQuerySchema.safeParse({
  ...
  const { items, total } = await listJobRunsDailyStats(query);
  return addRequestIdToResponse(
    Response.json({
      items,
      total,
